...
All tests share a single engine/connection per test to ensure the API
routes see the same tables and data that the test seeds.
"""
import json
...
class TestOrderStatusFlow:
    def test_full_status_chain(self, db_session):
        ...
        client.post("/api/v1/payments/mock/callback", json={"orderNo": order_no, "success": True})
        ...
        # Ship
        r = client.post(f"/api/v1/orders/{order_no}/ship")
        assert r.status_code == 200
...
class TestRefund:
    def _create_paid_order(self, db_session) -> tuple[str, str]:
        user, token = _seed_user(db_session)
        _seed_catalog(db_session, stock=100)
        _seed_address(db_session, user.id)
        _add_to_cart(db_session, user.id, "sku1", quantity=2)
    ...
    def test_refund_flow(self, db_session):
        order_no, token = self._create_paid_order(db_session)
        ...
        # Approve refund
        r = client.post(f"/api/v1/refunds/{refund_id}/approve")
        assert r.status_code == 200
    ...
    def test_refund_restores_stock(self, db_session):
        ...
        refund_id = r.json()["data"]["refundId"]
        client.post(f"/api/v1/refunds/{refund_id}/approve")
    ...
    def test_reject_refund_reverts_status(self, db_session):
        ...
        client.post(f"/api/v1/refunds/{refund_id}/reject")
